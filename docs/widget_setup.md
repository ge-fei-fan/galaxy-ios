...
- 收到 MQTT 新消息后可更新灵动岛/锁屏 Live Activity（iOS 16.1+）
- 灵动岛支持显示 MQTT **主题 + 消息内容 + 时间**
- 灵动岛开关在设置页可控，默认关闭
- 可选开启剪贴板监听：检测到新复制内容后发送通知，并可更新灵动岛

## 剪贴板监听（越狱/保活场景）

- 开关位置：`设置页 -> 启用剪贴板监听`（默认关闭）
- 开启后：原生侧定时检测 `UIPasteboard.general.string` 变化
- 检测到新内容时：
  - 发送本地通知展示复制内容
  - 更新小组件最新内容（topic 固定为 `clipboard/new`）
  - 若“启用灵动岛消息展示”已开启，则同步更新灵动岛

> 注意：该能力依赖保活状态，系统/环境差异可能影响持续性。

## 灵动岛开关说明

- 开关位置：`设置页 -> 启用灵动岛消息展示`
- 默认值：**关闭**
- 关闭时：收到 MQTT 仅更新小组件，不更新灵动岛
- 开启时：收到 MQTT 同时更新小组件和灵动岛
